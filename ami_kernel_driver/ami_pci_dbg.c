 ...
 */
char *pcie_device_header_to_str(pcie_header_struct *header)
{
	uint16_t device_id;

	if (header->vendor_id == PCIE_VENDOR_ID_XILINX) {
		device_id = header->device_id;
	} else if (header->vendor_id == PCIE_VENDOR_ID_MYRTLE && header->subsystem_vendor_id == PCIE_VENDOR_ID_XILINX) {
		device_id = header->subsystem_id;
	} else {
		device_id = 0xffff;
	}
...
void print_pcie_device_id(struct pci_dev *dev, pcie_header_struct *header)
{
	if (dev) {
		DEV_INFO(dev, "PCI_DEVICE_ID : %s", pcie_device_header_to_str(header));
  }
}
