...
static struct pci_device_id device_id[] = {
	{
		PCI_DEVICE(PCIE_VENDOR_ID, AMI_PCIE_DEVICE_ID_V70_PF0),
	},
	{
		PCI_DEVICE(PCIE_VENDOR_ID, AMI_PCIE_DEVICE_ID_VCK5000_PF0),
	},
	{
		PCI_DEVICE(PCIE_VENDOR_ID, AMI_PCIE_DEVICE_ID_V80),
	},
	{
		PCI_DEVICE(PCIE_VENDOR_ID, AMI_PCIE_DEVICE_ID_V80P),
	},
	{
		PCI_DEVICE(PCIE_VENDOR_ID_MYRTLE, PCIE_DEVICE_ID_MYRTLE),
	},
	{}
};
...
void pcie_device_remove(struct pci_dev *dev)
{
	...
	if (down_interruptible(&pf_dev->remove_sema)) {
		PR_WARN("Interrupted while waiting for pf_dev->remove_sema");
		return;
	}
...
int create_map_str(char **map_str, int *map_str_sz)
{
	...
	while ((pci = pci_get_device(PCIE_VENDOR_ID_MYRTLE, PCIE_DEVICE_ID_MYRTLE, pci)) != NULL) {
		if ((pci->driver) && (strcmp(pci->driver->name, DEFAULT_DEVICE_NAME) == 0)) {
			pf_dev = pci_get_drvdata(pci);
			sprintf(temp,
				"%02x:%02x.%1x %d %d\n",
...
static ssize_t myrtle_version_show(struct device_driver *drv, char *buf)
{
	if (!drv || !buf)
		return 0;
...
static DRIVER_ATTR_RO(myrtle_version);
...
int __init vmc_entry(void)
{
	...
	/* Create 'myrtle_version' attribute */
	ret = driver_create_file(&pcie_driver_core.driver, &driver_attr_myrtle_version);
	if (ret)
	...
	if (ret)
		goto remove_myrtle_version_attr;

	PR_INFO("Successfully loaded driver to the kernel");
	ami_debug_enabled = false;
	return SUCCESS;

remove_myrtle_version_attr:
	driver_remove_file(&pcie_driver_core.driver, &driver_attr_myrtle_version);
