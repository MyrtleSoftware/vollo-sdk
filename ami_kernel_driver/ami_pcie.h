...
#define DEV_DBG(pcie_dev, fmt, \
		arg ...)      do{ if (ami_debug_enabled) dev_dbg(&(pcie_dev->dev), "DEBUG           : " fmt "\n", \
								 ## arg); } while (0)
#define DEV_VDBG(pcie_dev, fmt, \
		 arg ...)      do{ if (ami_debug_enabled) dev_vdbg(&(pcie_dev->dev), \
								   "VERBOSE DEBUG           : " fmt "\n", ## arg); \
} while (0)
#define DEV_AMC_LOG(pcie_dev, fmt, arg ...)   dev_dbg(&(pcie_dev->dev), "AMC OUTPUT      : " fmt "\n", ## arg)

#define PCIE_VENDOR_ID_XILINX 0x10ee
#define PCIE_VENDOR_ID_MYRTLE 0x1ed9
#define PCIE_DEVICE_ID_MYRTLE 0x100a
...
#define get_pcie_command_intx_disable(command)  ((command & PCI_COMMAND_INTX_DISABLE) >> \
						 PCI_COMMAND_INTX_DISABLE_OFFSET)
...
#define get_pcie_status_sig_target_abort(status) ((status & PCI_STATUS_SIG_TARGET_ABORT) >> \
						  PCI_STATUS_SIG_TARGET_ABORT_OFFSET)
#define get_pcie_status_rec_target_abort(status) ((status & PCI_STATUS_REC_TARGET_ABORT) >> \
						  PCI_STATUS_REC_TARGET_ABORT_OFFSET)
#define get_pcie_status_rec_master_abort(status) ((status & PCI_STATUS_REC_MASTER_ABORT) >> \
						  PCI_STATUS_REC_MASTER_ABORT_OFFSET)
#define get_pcie_status_sig_system_error(status) ((status & PCI_STATUS_SIG_SYSTEM_ERROR) >> \
						  PCI_STATUS_SIG_SYSTEM_ERROR_OFFSET)
#define get_pcie_status_detected_parity(status)  ((status & PCI_STATUS_DETECTED_PARITY) >> \
						  PCI_STATUS_DETECTED_PARITY_OFFSET)

#define get_pcie_class_revision_id(class_revisio)           ((class_revision >> \
							      PCI_CLASS_REVISION_ID)&PCI_CLASS_REVISION_ID_MASK)
#define get_pcie_class_revision_prg_int(class_revisio)      ((class_revision >> \
							      PCI_CLASS_REVISION_PROG_IF)& \
							     PCI_CLASS_REVISION_PROG_IF_MASK)
#define get_pcie_class_revision_subclass(class_revisio)     ((class_revision >> \
							      PCI_CLASS_REVISION_SUBCLASS)& \
							     PCI_CLASS_REVISION_SUBCLASS_MASK)
#define get_pcie_class_revision_classcode(class_revisio)    ((class_revision >> \
							      PCI_CLASS_REVISION_CLASSCODE)& \
							     PCI_CLASS_REVISION_CLASSCODE_MASK)
 ...
 */
int read_pcie_bar(struct pci_dev *dev, uint8_t bar_idx, uint64_t offset, uint32_t num, uint32_t *val);
 ...
 * Return: 0 on success, negative error code otherwise
 */
int write_pcie_bar(struct pci_dev *dev, uint8_t bar_idx, uint64_t offset, uint32_t num, uint32_t *val);

/* Capabilities */

#define get_pcie_capability_cap_id(capability_read_buf)      ((capability_read_buf >> \
							       PCIE_CAP_ID_OFFSET)&PCIE_CAP_ID_MASK)
#define get_pcie_capability_next_cap(capability_read_buf)    ((capability_read_buf >> \
							       PCIE_CAP_NEXT_CAP_PTR_OFFSET)&PCIE_CAP_NEXT_CAP_PTR_MASK)
...
#define get_pcie_cap_pm_pme_support_d0_assert(cap_pm_pme_support)       ((cap_pm_pme_support >> \
									  PCIE_CAP_PM_PME_SUPPORT_D0_ASSERT_OFFSET)& \
									 PCIE_CAP_PM_PME_SUPPORT_D0_ASSERT_MASK)
#define get_pcie_cap_pm_pme_support_d1_assert(cap_pm_pme_support)       ((cap_pm_pme_support >> \
									  PCIE_CAP_PM_PME_SUPPORT_D1_ASSERT_OFFSET)& \
									 PCIE_CAP_PM_PME_SUPPORT_D1_ASSERT_MASK)
#define get_pcie_cap_pm_pme_support_d2_assert(cap_pm_pme_support)       ((cap_pm_pme_support >> \
									  PCIE_CAP_PM_PME_SUPPORT_D2_ASSERT_OFFSET)& \
									 PCIE_CAP_PM_PME_SUPPORT_D2_ASSERT_MASK)
#define get_pcie_cap_pm_pme_support_d3_hot_assert(cap_pm_pme_support)   ((cap_pm_pme_support >> \
									  PCIE_CAP_PM_PME_SUPPORT_D3_HOT_ASSERT_OFFSET)& \
									 PCIE_CAP_PM_PME_SUPPORT_D3_HOT_ASSERT_MASK)
#define get_pcie_cap_pm_pme_support_d3_cold_assert(cap_pm_pme_support)  ((cap_pm_pme_support >> \
									  PCIE_CAP_PM_PME_SUPPORT_D3_COLD_ASSERT_OFFSET) \
									 &PCIE_CAP_PM_PME_SUPPORT_D3_COLD_ASSERT_MASK)

#define get_pcie_cap_pm_status_cntrl(read_buf)               ((read_buf >> \
							       PCIE_CAP_PM_STATUS_CNTRL_OFFSET)& \
							      PCIE_CAP_PM_STATUS_CNTRL_MASK)
#define get_pcie_cap_pm_pmcsr_power_state(pm_status_cntrl)   ((pm_status_cntrl >> \
							       PCIE_CAP_PM_PMCSR_POWER_STATE_OFFSET)& \
							      PCIE_CAP_PM_PMCSR_POWER_STATE_MASK)
#define get_pcie_cap_pm_pmcsr_no_soft_reset(pm_status_cntrl) ((pm_status_cntrl >> \
							       PCIE_CAP_PM_PMCSR_NO_SOFT_RESET_OFFSET)& \
							      PCIE_CAP_PM_PMCSR_NO_SOFT_RESET_MASK)
#define get_pcie_cap_pm_pmcsr_pme_enable(pm_status_cntrl)    ((pm_status_cntrl >> \
							       PCIE_CAP_PM_PMCSR_PME_ENABLE_OFFSET)& \
							      PCIE_CAP_PM_PMCSR_PME_ENABLE_MASK)
#define get_pcie_cap_pm_pmcsr_data_sel(pm_status_cntrl)      ((pm_status_cntrl >> \
							       PCIE_CAP_PM_PMCSR_DATA_SEL_OFFSET)& \
							      PCIE_CAP_PM_PMCSR_DATA_SEL_MASK)
#define get_pcie_cap_pm_pmcsr_data_scale(pm_status_cntrl)    ((pm_status_cntrl >> \
							       PCIE_CAP_PM_PMCSR_DATA_SCALE_OFFSET)& \
							      PCIE_CAP_PM_PMCSR_DATA_SCALE_MASK)
#define get_pcie_cap_pm_pmcsr_pme_status(pm_status_cntrl)    ((pm_status_cntrl >> \
							       PCIE_CAP_PM_PMCSR_PME_STATUS_OFFSET)& \
							      PCIE_CAP_PM_PMCSR_PME_STATUS_MASK)

#define get_pcie_cap_pm_cntrl_bridge_ext(read_buf)               ((read_buf >> \
								   PCIE_CAP_PM_CNTRL_BRIDGE_EXT_OFFSET)& \
								  PCIE_CAP_PM_CNTRL_BRIDGE_EXT_MASK)
#define get_pcie_cap_pm_pmcsr_bse_b2_b3(pm_cntrl_bridge_ext)     ((pm_cntrl_bridge_ext >> \
								   PCIE_CAP_PM_PMCSR_BSE_B2_B3_OFFSET) & 0x01)
#define get_pcie_cap_pm_pmcsr_bse_bpcc_en(pm_cntrl_bridge_ext)   ((pm_cntrl_bridge_ext >> \
								   PCIE_CAP_PM_PMCSR_BSE_BPCC_EN_OFFSET) & 0x01)

#define get_pcie_cap_pm_data(read_buf) ((read_buf >> PCIE_CAP_PM_DATA_OFFSET)&PCIE_CAP_PM_CNTRL_BRIDGE_EXT_MASK)

#define get_pcie_cap_msi_config_status(read_buf)                     ((read_buf >> \
								       PCIE_CAP_MSI_OFFSET)&PCIE_CAP_MSI_REG_MASK)
#define get_pcie_cap_msi_msg_ctrl_config_en(msi_config_status)       ((msi_config_status >> \
								       PCIE_CAP_MSI_MSG_CTRL_CONFIG_EN_OFFSET)& \
...
#define get_pcie_cap_msi_msg_ctrl_per_vect_msk(msi_config_status)    ((msi_config_status >> \
								       PCIE_CAP_MSI_MSG_CTRL_PER_VECT_MSK_OFFSET)& \
								      PCIE_CAP_MSI_MSG_CTRL_PER_VECT_MSK_MASK)

#define get_pcie_exp_cap(read_buf)       ((read_buf >> PCIE_EXP_CAPABILITIES_OFFSET)&PCIE_EXP_CAPABILITIES_MASK)
#define get_pcie_cap_version(cap)        ((cap >> \
					   PCIE_EXP_CAPABILITIES_VERSION_OFFSET)&PCIE_EXP_CAPABILITIES_VERSION_MASK)
#define get_pcie_cap_dev_type(cap)       ((cap >> \
					   PCIE_EXP_CAPABILITIES_DEV_TYPE_OFFSET)&PCIE_EXP_CAPABILITIES_DEV_TYPE_MASK)
#define get_pcie_cap_slot_imp(cap)       ((cap >> \
					   PCIE_EXP_CAPABILITIES_SLOT_IMP_OFFSET)&PCIE_EXP_CAPABILITIES_SLOT_IMP_MASK)
#define get_pcie_cap_int_msg_num(cap)    ((cap >> \
					   PCIE_EXP_CAPABILITIES_INT_MSG_NUM_OFFSET)& \
					  PCIE_EXP_CAPABILITIES_INT_MSG_NUM_MASK)

#define get_pcie_cap_dev_max_payload(dev_cap)            ((dev_cap >> \
							   PCIE_CAP_DEV_CAPABILITIES_MAX_PAYLOAD_OFFSET)& \
							  PCIE_CAP_DEV_CAPABILITIES_MAX_PAYLOAD_MASK)
#define get_pcie_cap_dev_phantom_fn_sup(dev_cap)         ((dev_cap >> \
							   PCIE_CAP_DEV_CAPABILITIES_PHANTOM_FN_SUP_OFFSET)& \
							  PCIE_CAP_DEV_CAPABILITIES_PHANTOM_FN_SUP_MASK)
#define get_pcie_cap_dev_ext_tag_field_sup(dev_cap)      ((dev_cap >> \
							   PCIE_CAP_DEV_CAPABILITIES_EXT_TAG_FIELD_SUP_OFFSET)& \
							  PCIE_CAP_DEV_CAPABILITIES_EXT_TAG_FIELD_SUP_MASK)
#define get_pcie_cap_dev_l0_accpt_lat(dev_cap)           ((dev_cap >> \
							   PCIE_CAP_DEV_CAPABILITIES_ENDPT_L0_ACCPT_LAT_OFFSET)& \
							  PCIE_CAP_DEV_CAPABILITIES_ENDPT_L0_ACCPT_LAT_MASK)
#define get_pcie_cap_dev_endpt_l1_accpt_lat(dev_cap)     ((dev_cap >> \
							   PCIE_CAP_DEV_CAPABILITIES_ENDPT_L1_ACCPT_LAT_OFFSET)& \
							  PCIE_CAP_DEV_CAPABILITIES_ENDPT_L1_ACCPT_LAT_MASK)
#define get_pcie_cap_dev_role_based_err_rep(dev_cap)     ((dev_cap >> \
							   PCIE_CAP_DEV_CAPABILITIES_ROLE_BASED_ERR_REP_OFFSET)& \
							  PCIE_CAP_DEV_CAPABILITIES_ROLE_BASED_ERR_REP_MASK)
#define get_pcie_cap_dev_slot_pow_limit_val(dev_cap)     ((dev_cap >> \
							   PCIE_CAP_DEV_CAPABILITIES_CAP_SLOT_POW_LIMIT_VAL_OFFSET)& \
							  PCIE_CAP_DEV_CAPABILITIES_CAP_SLOT_POW_LIMIT_VAL_MASK)
#define get_pcie_cap_dev_slot_pow_limit_scale(dev_cap)   ((dev_cap >> \
							   PCIE_CAP_DEV_CAPABILITIES_CAP_SLOT_POW_LIMIT_SCALE_OFFSET)& \
							  PCIE_CAP_DEV_CAPABILITIES_CAP_SLOT_POW_LIMIT_SCALE_MASK)
#define get_pcie_cap_dev_fn_level_reset_cap(dev_cap)     ((dev_cap >> \
							   PCIE_CAP_DEV_CAPABILITIES_FN_LEVEL_RESET_CAP_OFFSET)& \
							  PCIE_CAP_DEV_CAPABILITIES_FN_LEVEL_RESET_CAP_MASK)

#define get_pcie_cap_dev_ctrl_correct_err_report_en(dev_ctrl)    ((dev_ctrl >> \
								   PCIE_CAP_DEV_CNTRL_CORRECT_ERR_REPORT_EN_OFFSET)& \
								  PCIE_CAP_DEV_CNTRL_CORRECT_ERR_REPORT_EN_MASK)
#define get_pcie_cap_dev_ctrl_non_fatal_err_report_en(dev_ctrl)  ((dev_ctrl >> \
								   PCIE_CAP_DEV_CNTRL_NON_FATAL_ERR_REPORT_EN_OFFSET)& \
								  PCIE_CAP_DEV_CNTRL_NON_FATAL_ERR_REPORT_EN_MASK)
#define get_pcie_cap_dev_ctrl_fatal_err_report_en(dev_ctrl)      ((dev_ctrl >> \
								   PCIE_CAP_DEV_CNTRL_FATAL_ERR_REPORT_EN_OFFSET)& \
								  PCIE_CAP_DEV_CNTRL_FATAL_ERR_REPORT_EN_MASK)
#define get_pcie_cap_dev_ctrl_unspprt_req_report_en(dev_ctrl)    ((dev_ctrl >> \
								   PCIE_CAP_DEV_CNTRL_UNSPPRT_REQ_REPORT_EN_OFFSET)& \
								  PCIE_CAP_DEV_CNTRL_UNSPPRT_REQ_REPORT_EN_MASK)
#define get_pcie_cap_dev_ctrl_en_relaxed_ord(dev_ctrl)           ((dev_ctrl >> \
								   PCIE_CAP_DEV_CNTRL_EN_RELAXED_ORD_OFFSET)& \
								  PCIE_CAP_DEV_CNTRL_EN_RELAXED_ORD_MASK)
#define get_pcie_cap_dev_ctrl_cap_max_payload_size(dev_ctrl)     ((dev_ctrl >> \
								   PCIE_CAP_DEV_CNTRL_PCIE_CAP_MAX_PAYLOAD_SIZE_OFFSET)& \
								  PCIE_CAP_DEV_CNTRL_PCIE_CAP_MAX_PAYLOAD_SIZE_MASK)
#define get_pcie_cap_dev_ctrl_ext_tag_filed_en(dev_ctrl)         ((dev_ctrl >> \
								   PCIE_CAP_DEV_CNTRL_EXT_TAG_FILED_EN_OFFSET)& \
								  PCIE_CAP_DEV_CNTRL_EXT_TAG_FILED_EN_MASK)
#define get_pcie_cap_dev_ctrl_phantom_fn_en(dev_ctrl)            ((dev_ctrl >> \
								   PCIE_CAP_DEV_CNTRL_PHANTOM_FN_EN_OFFSET)& \
								  PCIE_CAP_DEV_CNTRL_PHANTOM_FN_EN_MASK)
#define get_pcie_cap_dev_ctrl_aux_pwr_pm_en(dev_ctrl)            ((dev_ctrl >> \
								   PCIE_CAP_DEV_CNTRL_AUX_PWR_PM_EN_OFFSET)& \
								  PCIE_CAP_DEV_CNTRL_AUX_PWR_PM_EN_MASK)
#define get_pcie_cap_dev_ctrl_en_no_snoop(dev_ctrl)              ((dev_ctrl >> \
								   PCIE_CAP_DEV_CNTRL_EN_NO_SNOOP_OFFSET)& \
								  PCIE_CAP_DEV_CNTRL_EN_NO_SNOOP_MASK)
#define get_pcie_cap_dev_ctrl_max_read_req_size(dev_ctrl)        ((dev_ctrl >> \
								   PCIE_CAP_DEV_CNTRL_MAX_READ_REQ_SIZE_OFFSET)& \
								  PCIE_CAP_DEV_CNTRL_MAX_READ_REQ_SIZE_MASK)

#define get_pcie_cap_dev_status_correctable_err_det(dev_stat)    ((dev_stat >> \
								   PCIE_CAP_DEV_STATUS_CORRECTABLE_ERR_DET_OFFSET)& \
								  PCIE_CAP_DEV_STATUS_CORRECTABLE_ERR_DET_MASK)
#define get_pcie_cap_dev_status_nonfatal_err_det(dev_stat)       ((dev_stat >> \
								   PCIE_CAP_DEV_STATUS_NONFATAL_ERR_DET_OFFSET)& \
								  PCIE_CAP_DEV_STATUS_NONFATAL_ERR_DET_MASK)
#define get_pcie_cap_dev_status_fatal_err_det(dev_stat)          ((dev_stat >> \
								   PCIE_CAP_DEV_STATUS_FATAL_ERR_DET_OFFSET)& \
								  PCIE_CAP_DEV_STATUS_FATAL_ERR_DET_MASK)
#define get_pcie_cap_dev_status_unsup_req_det(dev_stat)          ((dev_stat >> \
								   PCIE_CAP_DEV_STATUS_UNSUP_REQ_DET_OFFSET)& \
								  PCIE_CAP_DEV_STATUS_UNSUP_REQ_DET_MASK)
#define get_pcie_cap_dev_status_aux_pwr_det(dev_stat)            ((dev_stat >> \
								   PCIE_CAP_DEV_STATUS_AUX_PWR_DET_OFFSET)& \
								  PCIE_CAP_DEV_STATUS_AUX_PWR_DET_MASK)
#define get_pcie_cap_dev_status_trans_pend(dev_stat)             ((dev_stat >> \
								   PCIE_CAP_DEV_STATUS_TRANS_PEND_OFFSET)& \
								  PCIE_CAP_DEV_STATUS_TRANS_PEND_MASK)

#define get_pcie_cap_link_cap_max_link_speed(cap_link)                       ((cap_link >> \
									       PCIE_CAP_LINK_CAPABILITIES_MAX_LINK_SPEED_OFFSET) \
									      & \
									      PCIE_CAP_LINK_CAPABILITIES_MAX_LINK_SPEED_MASK)
#define get_pcie_cap_link_cap_max_pcie_cap_link_width(cap_link)              ((cap_link >> \
									       PCIE_CAP_LINK_CAPABILITIES_MAX_PCIE_CAP_LINK_WIDTH_OFFSET) \
									      & \
									      PCIE_CAP_LINK_CAPABILITIES_MAX_PCIE_CAP_LINK_WIDTH_MASK)
#define get_pcie_cap_link_cap_pcie_cap_aspm_support(cap_link)                ((cap_link >> \
									       PCIE_CAP_LINK_CAPABILITIES_PCIE_CAP_ASPM_SUPPORT_OFFSET) \
									      & \
									      PCIE_CAP_LINK_CAPABILITIES_PCIE_CAP_ASPM_SUPPORT_MASK)
#define get_pcie_cap_link_cap_l0_exit_latency(cap_link)                      ((cap_link >> \
									       PCIE_CAP_LINK_CAPABILITIES_L0_EXIT_LATENCY_OFFSET) \
									      & \
									      PCIE_CAP_LINK_CAPABILITIES_L0_EXIT_LATENCY_MASK)
#define get_pcie_cap_link_cap_l1_exit_latency(cap_link)                      ((cap_link >> \
									       PCIE_CAP_LINK_CAPABILITIES_L1_EXIT_LATENCY_OFFSET) \
									      & \
									      PCIE_CAP_LINK_CAPABILITIES_L1_EXIT_LATENCY_MASK)
#define get_pcie_cap_link_cap_clk_pwr_mgmnt(cap_link)                        ((cap_link >> \
									       PCIE_CAP_LINK_CAPABILITIES_CLK_PWR_MGMNT_OFFSET) \
									      & \
									      PCIE_CAP_LINK_CAPABILITIES_CLK_PWR_MGMNT_MASK)
#define get_pcie_cap_link_cap_surp_down_err_report_cap(cap_link)             ((cap_link >> \
									       PCIE_CAP_LINK_CAPABILITIES_SURP_DOWN_ERR_REPORT_CAP_OFFSET) \
									      & \
									      PCIE_CAP_LINK_CAPABILITIES_SURP_DOWN_ERR_REPORT_CAP_MASK)
#define get_pcie_cap_link_cap_dlll_active_report_cap(cap_link)               ((cap_link >> \
									       PCIE_CAP_LINK_CAPABILITIES_DLLL_ACTIVE_REPORT_CAP_OFFSET) \
									      & \
									      PCIE_CAP_LINK_CAPABILITIES_DLLL_ACTIVE_REPORT_CAP_MASK)
#define get_pcie_cap_link_cap_link_bw_not_cap(cap_link)                      ((cap_link >> \
									       PCIE_CAP_LINK_CAPABILITIES_LINK_BW_NOT_CAP_OFFSET) \
									      & \
									      PCIE_CAP_LINK_CAPABILITIES_LINK_BW_NOT_CAP_MASK)
#define get_pcie_cap_link_cap_pcie_cap_aspm_optional_compliance(cap_link)    ((cap_link >> \
									       PCIE_CAP_LINK_CAPABILITIES_ASPM_OPTIONAL_COMPLIANCE_OFFSET) \
									      & \
									      PCIE_CAP_LINK_CAPABILITIES_ASPM_OPTIONAL_COMPLIANCE_MASK)
#define get_pcie_cap_link_cap_port_num(cap_link)                             ((cap_link >> \
									       PCIE_CAP_LINK_CAPABILITIES_PORT_NUM_OFFSET) \
									      &PCIE_CAP_LINK_CAPABILITIES_PORT_NUM_MASK)

#define get_pcie_cap_link_ctrl_pcie_cap_aspm_cntrl(link_ctrl)        ((link_ctrl >> \
								       PCIE_CAP_LINK_CNTRL_PCIE_CAP_ASPM_CNTRL_OFFSET)& \
								      PCIE_CAP_LINK_CNTRL_PCIE_CAP_ASPM_CNTRL_MASK)
#define get_pcie_cap_link_ctrl_rcb(link_ctrl)                        ((link_ctrl >> \
								       PCIE_CAP_LINK_CNTRL_RCB_OFFSET)& \
								      PCIE_CAP_LINK_CNTRL_RCB_MASK)
#define get_pcie_cap_link_ctrl_link_disable(link_ctrl)               ((link_ctrl >> \
								       PCIE_CAP_LINK_CNTRL_LINK_DISABLE_OFFSET)& \
								      PCIE_CAP_LINK_CNTRL_LINK_DISABLE_MASK)
#define get_pcie_cap_link_ctrl_retrain_link(link_ctrl)               ((link_ctrl >> \
								       PCIE_CAP_LINK_CNTRL_RETRAIN_LINK_OFFSET)& \
								      PCIE_CAP_LINK_CNTRL_RETRAIN_LINK_MASK)
#define get_pcie_cap_link_ctrl_common_clk_config(link_ctrl)          ((link_ctrl >> \
								       PCIE_CAP_LINK_CNTRL_COMMON_CLK_CONFIG_OFFSET)& \
								      PCIE_CAP_LINK_CNTRL_COMMON_CLK_CONFIG_MASK)
#define get_pcie_cap_link_ctrl_extended_synch(link_ctrl)             ((link_ctrl >> \
								       PCIE_CAP_LINK_CNTRL_EXTENDED_SYNCH_OFFSET)& \
								      PCIE_CAP_LINK_CNTRL_EXTENDED_SYNCH_MASK)
#define get_pcie_cap_link_ctrl_en_clk_pwr_mgmt(link_ctrl)            ((link_ctrl >> \
								       PCIE_CAP_LINK_CNTRL_EN_CLK_PWR_MGMT_OFFSET)& \
								      PCIE_CAP_LINK_CNTRL_EN_CLK_PWR_MGMT_MASK)
#define get_pcie_cap_link_ctrl_hardware_auto_width_dis(link_ctrl)    ((link_ctrl >> \
								       PCIE_CAP_LINK_CNTRL_HARDWARE_AUTO_WIDTH_DIS_OFFSET) \
								      &PCIE_CAP_LINK_CNTRL_HARDWARE_AUTO_WIDTH_DIS_MASK)
#define get_pcie_cap_link_ctrl_bw_mgmt_int_en(link_ctrl)             ((link_ctrl >> \
								       PCIE_CAP_LINK_CNTRL_BW_MGMT_INT_EN_OFFSET)& \
								      PCIE_CAP_LINK_CNTRL_BW_MGMT_INT_EN_MASK)
#define get_pcie_cap_link_ctrl_auto_bw_int_en(link_ctrl)             ((link_ctrl >> \
								       PCIE_CAP_LINK_CNTRL_AUTO_BW_INT_EN_OFFSET)& \
								      PCIE_CAP_LINK_CNTRL_AUTO_BW_INT_EN_MASK)
#define get_pcie_cap_link_ctrl_drs_signal_cntrl(link_ctrl)           ((link_ctrl >> \
								       PCIE_CAP_LINK_CNTRL_DRS_SIGNAL_CNTRL_OFFSET)& \
								      PCIE_CAP_LINK_CNTRL_DRS_SIGNAL_CNTRL_MASK)

#define get_pcie_cap_link_status_cur_link_speed(link_stat)           ((link_stat >> \
								       PCIE_CAP_LINK_STATUS_CUR_LINK_SPEED_OFFSET)& \
								      PCIE_CAP_LINK_STATUS_CUR_LINK_SPEED_MASK)
#define get_pcie_cap_link_status_neg_pcie_cap_link_width(link_stat)  ((link_stat >> \
								       PCIE_CAP_LINK_STATUS_NEG_PCIE_CAP_LINK_WIDTH_OFFSET) \
								      &PCIE_CAP_LINK_STATUS_NEG_PCIE_CAP_LINK_WIDTH_MASK)
#define get_pcie_cap_link_status_link_training(link_stat)            ((link_stat >> \
								       PCIE_CAP_LINK_STATUS_LINK_TRAINING_OFFSET)& \
								      PCIE_CAP_LINK_STATUS_LINK_TRAINING_MASK)
#define get_pcie_cap_link_status_slot_clk_config(link_stat)          ((link_stat >> \
								       PCIE_CAP_LINK_STATUS_SLOT_CLK_CONFIG_OFFSET)& \
								      PCIE_CAP_LINK_STATUS_SLOT_CLK_CONFIG_MASK)
#define get_pcie_cap_link_status_dll_active(link_stat)               ((link_stat >> \
								       PCIE_CAP_LINK_STATUS_DLL_ACTIVE_OFFSET)& \
								      PCIE_CAP_LINK_STATUS_DLL_ACTIVE_MASK)
#define get_pcie_cap_link_status_link_bw_mgmt_status(link_stat)      ((link_stat >> \
								       PCIE_CAP_LINK_STATUS_LINK_BW_MGMT_STATUS_OFFSET)& \
								      PCIE_CAP_LINK_STATUS_LINK_BW_MGMT_STATUS_MASK)
#define get_pcie_cap_link_status_link_auto_bw_status(link_stat)      ((link_stat >> \
								       PCIE_CAP_LINK_STATUS_LINK_AUTO_BW_STATUS_OFFSET)& \
								      PCIE_CAP_LINK_STATUS_LINK_AUTO_BW_STATUS_MASK)

#define get_pcie_cap_slot_cap_attention_button_present(slot_cap)     ((slot_cap >> \
								       PCIE_CAP_SLOT_CAPABILITIES_ATTENTION_BUTTON_PRESENT_OFFSET) \
								      & \
								      PCIE_CAP_SLOT_CAPABILITIES_ATTENTION_BUTTON_PRESENT_MASK)
#define get_pcie_cap_slot_cap_power_controller_present(slot_cap)     ((slot_cap >> \
								       PCIE_CAP_SLOT_CAPABILITIES_POWER_CONTROLLER_PRESENT_OFFSET) \
								      & \
								      PCIE_CAP_SLOT_CAPABILITIES_POWER_CONTROLLER_PRESENT_MASK)
#define get_pcie_cap_slot_cap_mrl_sensor_present(slot_cap)           ((slot_cap >> \
								       PCIE_CAP_SLOT_CAPABILITIES_MRL_SENSOR_PRESENT_OFFSET) \
								      & \
								      PCIE_CAP_SLOT_CAPABILITIES_MRL_SENSOR_PRESENT_MASK)
#define get_pcie_cap_slot_cap_attention_indicator_present(slot_cap)  ((slot_cap >> \
								       PCIE_CAP_SLOT_CAPABILITIES_ATTENTION_INDICATOR_PRESENT_OFFSET) \
								      & \
								      PCIE_CAP_SLOT_CAPABILITIES_ATTENTION_INDICATOR_PRESENT_MASK)
#define get_pcie_cap_slot_cap_power_indicator_present(slot_cap)      ((slot_cap >> \
								       PCIE_CAP_SLOT_CAPABILITIES_POWER_INDICATOR_PRESENT_OFFSET) \
								      & \
								      PCIE_CAP_SLOT_CAPABILITIES_POWER_INDICATOR_PRESENT_MASK)
#define get_pcie_cap_slot_cap_hot_plug_surprise(slot_cap)            ((slot_cap >> \
								       PCIE_CAP_SLOT_CAPABILITIES_HOT_PLUG_SURPRISE_OFFSET) \
								      &PCIE_CAP_SLOT_CAPABILITIES_HOT_PLUG_SURPRISE_MASK)
#define get_pcie_cap_slot_cap_hot_plug_capable(slot_cap)             ((slot_cap >> \
								       PCIE_CAP_SLOT_CAPABILITIES_HOT_PLUG_CAPABLE_OFFSET) \
								      &PCIE_CAP_SLOT_CAPABILITIES_HOT_PLUG_CAPABLE_MASK)
#define get_pcie_cap_slot_cap_slot_power_limit_val(slot_cap)         ((slot_cap >> \
								       PCIE_CAP_SLOT_CAPABILITIES_SLOT_POWER_LIMIT_VAL_OFFSET) \
								      & \
								      PCIE_CAP_SLOT_CAPABILITIES_SLOT_POWER_LIMIT_VAL_MASK)
#define get_pcie_cap_slot_cap_slot_power_limit_scale(slot_cap)       ((slot_cap >> \
								       PCIE_CAP_SLOT_CAPABILITIES_SLOT_POWER_LIMIT_SCALE_OFFSET) \
								      & \
								      PCIE_CAP_SLOT_CAPABILITIES_SLOT_POWER_LIMIT_SCALE_MASK)
#define get_pcie_cap_slot_cap_electromag_intrlck_present(slot_cap)   ((slot_cap >> \
								       PCIE_CAP_SLOT_CAPABILITIES_ELECTROMAG_INTRLCK_PRESENT_OFFSET) \
								      & \
								      PCIE_CAP_SLOT_CAPABILITIES_ELECTROMAG_INTRLCK_PRESENT_MASK)
#define get_pcie_cap_slot_cap_no_cmd_comp_support(slot_cap)          ((slot_cap >> \
								       PCIE_CAP_SLOT_CAPABILITIES_NO_CMD_COMP_SUPPORT_OFFSET) \
								      & \
								      PCIE_CAP_SLOT_CAPABILITIES_NO_CMD_COMP_SUPPORT_MASK)

#define get_pcie_cap_slot_ctrl_attention_button_pressed_en(slot_ctrl)    ((slot_ctrl >> \
									   PCIE_CAP_SLOT_CNTRL_ATTENTION_BUTTON_PRESSED_EN_OFFSET) \
									  & \
									  PCIE_CAP_SLOT_CNTRL_ATTENTION_BUTTON_PRESSED_EN_MASK)
#define get_pcie_cap_slot_ctrl_pwr_fault_det_en(slot_ctrl)               ((slot_ctrl >> \
									   PCIE_CAP_SLOT_CNTRL_PWR_FAULT_DET_EN_OFFSET)& \
									  PCIE_CAP_SLOT_CNTRL_PWR_FAULT_DET_EN_MASK)
#define get_pcie_cap_slot_ctrl_mrl_sensor_changed_en(slot_ctrl)          ((slot_ctrl >> \
									   PCIE_CAP_SLOT_CNTRL_MRL_SENSOR_CHANGED_EN_OFFSET) \
									  & \
									  PCIE_CAP_SLOT_CNTRL_MRL_SENSOR_CHANGED_EN_MASK)
#define get_pcie_cap_slot_ctrl_present_det_change_en(slot_ctrl)          ((slot_ctrl >> \
									   PCIE_CAP_SLOT_CNTRL_PRESENT_DET_CHANGE_EN_OFFSET) \
									  & \
									  PCIE_CAP_SLOT_CNTRL_PRESENT_DET_CHANGE_EN_MASK)
#define get_pcie_cap_slot_ctrl_cmd_comp_int_en(slot_ctrl)                ((slot_ctrl >> \
									   PCIE_CAP_SLOT_CNTRL_CMD_COMP_INT_EN_OFFSET)& \
									  PCIE_CAP_SLOT_CNTRL_CMD_COMP_INT_EN_MASK)
#define get_pcie_cap_slot_ctrl_hot_plug_int_en(slot_ctrl)                ((slot_ctrl >> \
									   PCIE_CAP_SLOT_CNTRL_HOT_PLUG_INT_EN_OFFSET)& \
									  PCIE_CAP_SLOT_CNTRL_HOT_PLUG_INT_EN_MASK)
#define get_pcie_cap_slot_ctrl_cntrl_attn_indicator_cntrl(slot_ctrl)     ((slot_ctrl >> \
									   PCIE_CAP_SLOT_CNTRL_ATTN_INDICATOR_CNTRL_OFFSET) \
									  &PCIE_CAP_SLOT_CNTRL_ATTN_INDICATOR_CNTRL_MASK)
#define get_pcie_cap_slot_ctrl_pwr_indicator_cntrl(slot_ctrl)            ((slot_ctrl >> \
									   PCIE_CAP_SLOT_CNTRL_PWR_INDICATOR_CNTRL_OFFSET) \
									  &PCIE_CAP_SLOT_CNTRL_PWR_INDICATOR_CNTRL_MASK)
#define get_pcie_cap_slot_ctrl_pwr_controller_cntrl(slot_ctrl)           ((slot_ctrl >> \
									   PCIE_CAP_SLOT_CNTRL_PWR_CONTROLLER_CNTRL_OFFSET) \
									  &PCIE_CAP_SLOT_CNTRL_PWR_CONTROLLER_CNTRL_MASK)
#define get_pcie_cap_slot_ctrl_electromec_interlock_cntrl(slot_ctrl)     ((slot_ctrl >> \
									   PCIE_CAP_SLOT_CNTRL_ELECTROMEC_INTERLOCK_CNTRL_OFFSET) \
									  & \
									  PCIE_CAP_SLOT_CNTRL_ELECTROMEC_INTERLOCK_CNTRL_MASK)
#define get_pcie_cap_slot_ctrl_dlls_changed_en(slot_ctrl)                ((slot_ctrl >> \
									   PCIE_CAP_SLOT_CNTRL_DLLS_CHANGED_EN_OFFSET)& \
									  PCIE_CAP_SLOT_CNTRL_DLLS_CHANGED_EN_MASK)

#define get_pcie_cap_slot_status_attention_but_pressed(slot_stat)        ((slot_stat >> \
									   PCIE_CAP_SLOT_STATUS_ATTENTION_BUT_PRESSED_OFFSET) \
									  & \
									  PCIE_CAP_SLOT_STATUS_ATTENTION_BUT_PRESSED_MASK)
#define get_pcie_cap_slot_status_status_pwr_fault_det(slot_stat)         ((slot_stat >> \
									   PCIE_CAP_SLOT_STATUS_PWR_FAULT_DET_OFFSET)& \
									  PCIE_CAP_SLOT_STATUS_PWR_FAULT_DET_MASK)
#define get_pcie_cap_slot_status_mrl_sensor_chng(slot_stat)              ((slot_stat >> \
									   PCIE_CAP_SLOT_STATUS_MRL_SENSOR_CHNG_OFFSET)& \
									  PCIE_CAP_SLOT_STATUS_MRL_SENSOR_CHNG_MASK)
#define get_pcie_cap_slot_status_presence_det_chng(slot_stat)            ((slot_stat >> \
									   PCIE_CAP_SLOT_STATUS_PRESENCE_DET_CHNG_OFFSET) \
									  &PCIE_CAP_SLOT_STATUS_PRESENCE_DET_CHNG_MASK)
#define get_pcie_cap_slot_status_cmd_comp(slot_stat)                     ((slot_stat >> \
									   PCIE_CAP_SLOT_STATUS_CMD_COMP_OFFSET)& \
									  PCIE_CAP_SLOT_STATUS_CMD_COMP_MASK)
#define get_pcie_cap_slot_status_mrl_sensor_state(slot_stat)             ((slot_stat >> \
									   PCIE_CAP_SLOT_STATUS_MRL_SENSOR_STATE_OFFSET) \
									  &PCIE_CAP_SLOT_STATUS_MRL_SENSOR_STATE_MASK)
#define get_pcie_cap_slot_status_presence_detect_state(slot_stat)        ((slot_stat >> \
									   PCIE_CAP_SLOT_STATUS_PRESENCE_DETECT_STATE_OFFSET) \
									  & \
									  PCIE_CAP_SLOT_STATUS_PRESENCE_DETECT_STATE_MASK)
#define get_pcie_cap_slot_status_electromec_interlock_status(slot_stat)  ((slot_stat >> \
									   PCIE_CAP_SLOT_STATUS_ELECTROMEC_INTERLOCK_STATUS_OFFSET) \
									  & \
									  PCIE_CAP_SLOT_STATUS_ELECTROMEC_INTERLOCK_STATUS_MASK)
#define get_pcie_cap_slot_status_dll_state_changed(slot_stat)            ((slot_stat >> \
									   PCIE_CAP_SLOT_STATUS_DLL_STATE_CHANGED_OFFSET) \
									  &PCIE_CAP_SLOT_STATUS_DLL_STATE_CHANGED_MASK)

#define get_pcie_cap_root_ctrl_err_corr_err_en(root_ctrl)            ((root_ctrl >> \
								       PCIE_CAP_ROOT_CNTRL_ERR_CORR_ERR_EN_OFFSET)& \
								      PCIE_CAP_ROOT_CNTRL_ERR_CORR_ERR_EN_MASK)
#define get_pcie_cap_root_ctrl_err_non_fatal_err(root_ctrl)          ((root_ctrl >> \
								       PCIE_CAP_ROOT_CNTRL_ERR_NON_FATAL_ERR_EN_OFFSET)& \
								      PCIE_CAP_ROOT_CNTRL_ERR_NON_FATAL_ERR_EN_MASK)
#define get_pcie_cap_root_ctrl_err_fatal_err_en(root_ctrl)           ((root_ctrl >> \
								       PCIE_CAP_ROOT_CNTRL_ERR_FATAL_ERR_EN_OFFSET)& \
								      PCIE_CAP_ROOT_CNTRL_ERR_FATAL_ERR_EN_MASK)
#define get_pcie_cap_root_ctrl_pme_int_en(root_ctrl)                 ((root_ctrl >> \
								       PCIE_CAP_ROOT_CNTRL_PME_INT_EN_OFFSET)& \
								      PCIE_CAP_ROOT_CNTRL_PME_INT_EN_MASK)
#define get_pcie_cap_root_ctrl_crs_software_visibility_en(root_ctrl) ((root_ctrl >> \
								       PCIE_CAP_ROOT_CNTRL_CRS_SOFTWARE_VISIBILITY_EN_OFFSET) \
								      & \
								      PCIE_CAP_ROOT_CNTRL_CRS_SOFTWARE_VISIBILITY_EN_MASK)

#define get_pcie_cap_root_cap_crs_soft_vis(root_cap) ((root_cap >> \
						       PCIE_CAP_ROOT_CAPABILITIES_CRS_SOFT_VIS_OFFSET)& \
						      PCIE_CAP_ROOT_CAPABILITIES_CRS_SOFT_VIS_MASK)

#define get_pcie_cap_root_status_pme_status(root_stat)   ((root_stat >> \
							   PCIE_CAP_ROOT_STATUS_PME_STATUS_OFFSET)& \
							  PCIE_CAP_ROOT_STATUS_PME_STATUS_MASK)
#define get_pcie_cap_root_status_pme_req_i(root_stat)    ((root_stat >> \
							   PCIE_CAP_ROOT_STATUS_PME_REQ_ID_OFFSET)& \
							  PCIE_CAP_ROOT_STATUS_PME_REQ_ID_MASK)
#define get_pcie_cap_root_status_pme_pend(root_stat)     ((root_stat >> \
							   PCIE_CAP_ROOT_STATUS_PME_PEND_OFFSET)& \
							  PCIE_CAP_ROOT_STATUS_PME_PEND_MASK)

#define get_pcie_cap_dev_cap_2_timeout_range_sup(dev_cap)            ((dev_cap >> \
								       PCIE_CAP_DEV_CAPABILITIES_2_COMP_TIMEOUT_RANGE_SUP_OFFSET) \
								      & \
								      PCIE_CAP_DEV_CAPABILITIES_2_COMP_TIMEOUT_RANGE_SUP_MASK)
#define get_pcie_cap_dev_cap_2_comp_timeout_disable_sup(dev_cap)     ((dev_cap >> \
								       PCIE_CAP_DEV_CAPABILITIES_2_COMP_TIMEOUT_DISABLE_SUP_OFFSET) \
								      & \
								      PCIE_CAP_DEV_CAPABILITIES_2_COMP_TIMEOUT_DISABLE_SUP_MASK)
#define get_pcie_cap_dev_cap_2_ari_forward_sup(dev_cap)              ((dev_cap >> \
								       PCIE_CAP_DEV_CAPABILITIES_2_ARI_FORWARD_SUP_OFFSET) \
								      &PCIE_CAP_DEV_CAPABILITIES_2_ARI_FORWARD_SUP_MASK)
#define get_pcie_cap_dev_cap_2_atomic_op_routing_sup(dev_cap)        ((dev_cap >> \
								       PCIE_CAP_DEV_CAPABILITIES_2_ATOMIC_OP_ROUTING_SUP_OFFSET) \
								      & \
								      PCIE_CAP_DEV_CAPABILITIES_2_ATOMIC_OP_ROUTING_SUP_MASK)
#define get_pcie_cap_dev_cap_2_32_bit_atomic_op_comp_sup(dev_cap)    ((dev_cap >> \
								       PCIE_CAP_DEV_CAPABILITIES_2_32_BIT_ATOMIC_OP_COMP_SUP_OFFSET) \
								      & \
								      PCIE_CAP_DEV_CAPABILITIES_2_32_BIT_ATOMIC_OP_COMP_SUP_MASK)
#define get_pcie_cap_dev_cap_2_64_bit_atomic_op_comp_sup(dev_cap)    ((dev_cap >> \
								       PCIE_CAP_DEV_CAPABILITIES_2_64_BIT_ATOMIC_OP_COMP_SUP_OFFSET) \
								      & \
								      PCIE_CAP_DEV_CAPABILITIES_2_64_BIT_ATOMIC_OP_COMP_SUP_MASK)
#define get_pcie_cap_dev_cap_2_128_bit_cas_comp_sup(dev_cap)         ((dev_cap >> \
								       PCIE_CAP_DEV_CAPABILITIES_2_128_BIT_CAS_COMP_SUP_OFFSET) \
								      & \
								      PCIE_CAP_DEV_CAPABILITIES_2_128_BIT_CAS_COMP_SUP_MASK)
#define get_pcie_cap_dev_cap_2_no_ro_en_pr_pas(dev_cap)              ((dev_cap >> \
								       PCIE_CAP_DEV_CAPABILITIES_2_NO_RO_EN_PR_PASS_OFFSET) \
								      & \
								      PCIE_CAP_DEV_CAPABILITIES_2_NO_RO_EN_PR_PR_PASS_MASK)
#define get_pcie_cap_dev_cap_2_ltr_mech_sup(dev_cap)                 ((dev_cap >> \
								       PCIE_CAP_DEV_CAPABILITIES_2_LTR_MECH_SUP_OFFSET)& \
								      PCIE_CAP_DEV_CAPABILITIES_2_LTR_MECH_SUP_MASK)
#define get_pcie_cap_dev_cap_2_tph_comp_sup(dev_cap)                 ((dev_cap >> \
								       PCIE_CAP_DEV_CAPABILITIES_2_TPH_COMP_SUP_OFFSET)& \
								      PCIE_CAP_DEV_CAPABILITIES_2_TPH_COMP_SUP_MASK)
#define get_pcie_cap_dev_cap_2_obff_sup(dev_cap)                     ((dev_cap >> \
								       PCIE_CAP_DEV_CAPABILITIES_2_OBFF_SUP_OFFSET)& \
								      PCIE_CAP_DEV_CAPABILITIES_2_OBFF_SUP_MASK)
#define get_pcie_cap_dev_cap_2_ext_fmt_field_sup(dev_cap)            ((dev_cap >> \
								       PCIE_CAP_DEV_CAPABILITIES_2_EXT_FMT_FIELD_SUP_OFFSET) \
								      & \
								      PCIE_CAP_DEV_CAPABILITIES_2_EXT_FMT_FIELD_SUP_MASK)
#define get_pcie_cap_dev_cap_2_end_tlp_prefix_sup(dev_cap)           ((dev_cap >> \
								       PCIE_CAP_DEV_CAPABILITIES_2_END_TLP_PREFIX_SUP_OFFSET) \
								      & \
								      PCIE_CAP_DEV_CAPABILITIES_2_END_TLP_PREFIX_SUP_MASK)
#define get_pcie_cap_dev_cap_2_max_end_tlp_prefixes(dev_cap)         ((dev_cap >> \
								       PCIE_CAP_DEV_CAPABILITIES_2_MAX_END_TLP_PREFIXES_OFFSET) \
								      & \
								      PCIE_CAP_DEV_CAPABILITIES_2_MAX_END_END_TLP_PREFIXES_MASK)

#define get_pcie_cap_dev_ctrl_2_comp_timeout_val(dev_ctrl)       ((dev_ctrl >> \
								   PCIE_CAP_DEV_CNTRL_2_COMP_TIMEOUT_VAL_OFFSET)& \
								  PCIE_CAP_DEV_CNTRL_2_COMP_TIMEOUT_VAL_MASK)
#define get_pcie_cap_dev_ctrl_2_comp_timeout_disable(dev_ctrl)   ((dev_ctrl >> \
								   PCIE_CAP_DEV_CNTRL_2_COMP_TIMEOUT_DISABLE_OFFSET)& \
								  PCIE_CAP_DEV_CNTRL_2_COMP_TIMEOUT_DISABLE_MASK)
#define get_pcie_cap_dev_ctrl_2_ari_frwd_en(dev_ctrl)            ((dev_ctrl >> \
								   PCIE_CAP_DEV_CNTRL_2_ARI_FRWD_EN_OFFSET)& \
								  PCIE_CAP_DEV_CNTRL_2_ARI_FRWD_EN_MASK)
#define get_pcie_cap_dev_ctrl_2_atomic_op_req_en(dev_ctrl)       ((dev_ctrl >> \
								   PCIE_CAP_DEV_CNTRL_2_ATOMIC_OP_REQ_EN_OFFSET)& \
								  PCIE_CAP_DEV_CNTRL_2_ATOMIC_OP_REQ_EN_MASK)
#define get_pcie_cap_dev_ctrl_2_atomic_op_egress_block(dev_ctrl) ((dev_ctrl >> \
								   PCIE_CAP_DEV_CNTRL_2_ATOMIC_OP_EGRESS_BLOCK_OFFSET)& \
								  PCIE_CAP_DEV_CNTRL_2_ATOMIC_OP_EGRESS_BLOCK_MASK)
#define get_pcie_cap_dev_ctrl_2_ido_req_en(dev_ctrl)             ((dev_ctrl >> \
								   PCIE_CAP_DEV_CNTRL_2_IDO_REQ_EN_OFFSET)& \
								  PCIE_CAP_DEV_CNTRL_2_IDO_REQ_EN_MASK)
#define get_pcie_cap_dev_ctrl_2_ido_comp_en(dev_ctrl)            ((dev_ctrl >> \
								   PCIE_CAP_DEV_CNTRL_2_IDO_COMP_EN_OFFSET)& \
								  PCIE_CAP_DEV_CNTRL_2_IDO_COMP_EN_MASK)
#define get_pcie_cap_dev_ctrl_2_ltr_mech_en(dev_ctrl)            ((dev_ctrl >> \
								   PCIE_CAP_DEV_CNTRL_2_LTR_MECH_EN_OFFSET)& \
								  PCIE_CAP_DEV_CNTRL_2_LTR_MECH_EN_MASK)
#define get_pcie_cap_dev_ctrl_2_obff_en(dev_ctrl)                ((dev_ctrl >> \
								   PCIE_CAP_DEV_CNTRL_2_OBFF_EN_OFFSET)& \
								  PCIE_CAP_DEV_CNTRL_2_OBFF_EN_MASK)
#define get_pcie_cap_dev_ctrl_2_end_tlp_prefix_block(dev_ctrl)   ((dev_ctrl >> \
								   PCIE_CAP_DEV_CNTRL_2_END_TLP_PREFIX_BLOCK_OFFSET)& \
								  PCIE_CAP_DEV_CNTRL_2_END_TLP_PREFIX_BLOCK_MASK)

#define get_pcie_cap_link_cap_2_supp_link_speed_vector(link_cap) ((link_cap >> \
								   PCIE_CAP_LINK_CAPABILITIES_2_SUPP_LINK_SPEED_VECTOR_OFFSET) \
								  & \
								  PCIE_CAP_LINK_CAPABILITIES_2_SUPP_LINK_SPEED_VECTOR_MASK)
#define get_pcie_cap_link_cap_2_crosslink_sup(link_cap)          ((link_cap >> \
								   PCIE_CAP_LINK_CAPABILITIES_2_CROSSLINK_SUP_OFFSET)& \
								  PCIE_CAP_LINK_CAPABILITIES_2_CROSSLINK_SUP_MASK)

#define get_pcie_cap_link_ctrl_2_target_link_speed(link_ctrl)        ((link_ctrl >> \
								       PCIE_CAP_LINK_CNTRL_2_TARGET_LINK_SPEED_OFFSET)& \
								      PCIE_CAP_LINK_CNTRL_2_TARGET_LINK_SPEED_MASK)
#define get_pcie_cap_link_ctrl_2_enter_complianc(link_ctrl)          ((link_ctrl >> \
								       PCIE_CAP_LINK_CNTRL_2_ENTER_COMPLIANCE_OFFSET)& \
								      PCIE_CAP_LINK_CNTRL_2_ENTER_COMPLIANCE_MASK)
#define get_pcie_cap_link_ctrl_2_hw_auto_speed_disable(link_ctrl)    ((link_ctrl >> \
								       PCIE_CAP_LINK_CNTRL_2_HW_AUTO_SPEED_DISABLE_OFFSET) \
								      &PCIE_CAP_LINK_CNTRL_2_HW_AUTO_SPEED_DISABLE_MASK)
#define get_pcie_cap_link_ctrl_2_sel_de_emphasis(link_ctrl)          ((link_ctrl >> \
								       PCIE_CAP_LINK_CNTRL_2_SEL_DE_EMPHASIS_OFFSET)& \
								      PCIE_CAP_LINK_CNTRL_2_SEL_DE_EMPHASIS_MASK)
#define get_pcie_cap_link_ctrl_2_transmit_margin(link_ctrl)          ((link_ctrl >> \
								       PCIE_CAP_LINK_CNTRL_2_TRANSMIT_MARGIN_OFFSET)& \
								      PCIE_CAP_LINK_CNTRL_2_TRANSMIT_MARGIN_MASK)
#define get_pcie_cap_link_ctrl_2_enter_modified_comp(link_ctrl)      ((link_ctrl >> \
								       PCIE_CAP_LINK_CNTRL_2_ENTER_MODIFIED_COMP_OFFSET) \
								      &PCIE_CAP_LINK_CNTRL_2_ENTER_MODIFIED_COMP_MASK)
#define get_pcie_cap_link_ctrl_2_compliance_sos(link_ctrl)           ((link_ctrl >> \
								       PCIE_CAP_LINK_CNTRL_2_COMPLIANCE_SOS_OFFSET)& \
								      PCIE_CAP_LINK_CNTRL_2_COMPLIANCE_SOS_MASK)
#define get_pcie_cap_link_ctrl_2_compliance_preset(link_ctrl)        ((link_ctrl >> \
								       PCIE_CAP_LINK_CNTRL_2_COMPLIANCE_PRESET_OFFSET)& \
								      PCIE_CAP_LINK_CNTRL_2_COMPLIANCE_PRESET_MASK)

#define get_pcie_cap_link_status_2_cur_deemphasis_level(link_stat)      ((link_stat >> \
									  PCIE_CAP_LINK_STATUS2_CUR_DEEMPHASIS_LEVEL_OFFSET) \
									 & \
									 PCIE_CAP_LINK_STATUS2_CUR_DEEMPHASIS_LEVEL_MASK)
#define get_pcie_cap_link_status_2_equalization_comp(link_stat)         ((link_stat >> \
									  PCIE_CAP_LINK_STATUS2_EQUALIZATION_COMP_OFFSET) \
									 &PCIE_CAP_LINK_STATUS2_EQUALIZATION_COMP_MASK)
#define get_pcie_cap_link_status_2_equalization_p1_suc(link_stat)       ((link_stat >> \
									  PCIE_CAP_LINK_STATUS2_EQUALIZATION_P1_SUC_OFFSET) \
									 &PCIE_CAP_LINK_STATUS2_EQUALIZATION_P1_SUC_MASK)
#define get_pcie_cap_link_status_2_equalization_p2_suc(link_stat)       ((link_stat >> \
									  PCIE_CAP_LINK_STATUS2_EQUALIZATION_P2_SUC_OFFSET) \
									 &PCIE_CAP_LINK_STATUS2_EQUALIZATION_P2_SUC_MASK)
#define get_pcie_cap_link_status_2_equalization_p3_suc(link_stat)     ((link_stat >> \
									PCIE_CAP_LINK_STATUS2_EQUALIZATION_P3_SUC_OFFSET) \
								       &PCIE_CAP_LINK_STATUS2_EQUALIZATION_P3_SUC_MASK)
#define get_pcie_cap_link_status_2_link_equalization_req(link_stat)     ((link_stat >> \
									  PCIE_CAP_LINK_STATUS2_LINK_EQUALIZATION_REQ_OFFSET) \
									 & \
									 PCIE_CAP_LINK_STATUS2_LINK_EQUALIZATION_REQ_MASK)

int read_pcie_capabilities(struct pci_dev *dev, pcie_header_struct *header, pcie_cap_struct **cap);

/* Extended Capabilities */

#define get_pcie_capability_ext_cap_id(ext_header_read_buf)      ((ext_header_read_buf >> \
								   PCIE_EXT_CAP_ID_OFFSET)&PCIE_EXT_CAP_ID_MASK)
#define get_pcie_capability_ext_cap_version(ext_header_read_buf) ((ext_header_read_buf >> \
								   PCIE_EXT_CAP_VERSION_OFFSET)& \
								  PCIE_EXT_CAP_VERSION_MASK)
#define get_pcie_capability_ext_cap_next(ext_header_read_buf)    ((ext_header_read_buf >> \
								   PCIE_EXT_CAP_NEXT_OFFSET)&PCIE_EXT_CAP_NEXT_MASK)
